...
// Zwevende live-chat met de recruiters (pmchat-threads). Bruikbaar op login- én
// toegang-geweigerd-scherm, zodat (afgewezen) bezoekers contact kunnen leggen.
      ...
      <div style={{ padding: '0.6rem 0.85rem', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        {name && (
          <div style={{ marginTop: '0.35rem', fontSize: '0.56rem', color: 'var(--text-dim)' }}>
            Je chat als <span style={{ color: nameColor(name) }}>{name}</span> · <button onClick={() => { localStorage.removeItem('pm_name'); setName('') }} style={{ background: 'none', border: 'none', color: 'var(--text-dim)', textDecoration: 'underline', cursor: 'pointer', fontSize: '0.56rem', padding: 0 }}>wijzig</button>
          </div>
        )}
      </div>
      </div>
