...
const FEED={ok:true,region:'Cobalt Edge',region_id:10000053,aantal:2,kwetsbaar_nu:1,onder_aanval:1,ours_count:1,ours_attack:1,bijgewerkt:new Date().toISOString(),
  rows:[
    {structure_id:1,system_id:30001780,type:'IHUB',type_full:'Infrastructure Hub',system:'GQLB-V',sec:-0.4,alliance_id:99011990,alliance:'Insidious.',ours:true,adm:1.4,status:'campaign',when:iso(140),campaign:true,defender:'Insidious.',defender_score:60,attackers_score:40,moved:true,d_def:-8,d_att:8,trend:'att'},
    {structure_id:2,system_id:30000208,type:'IHUB',type_full:'Infrastructure Hub',system:'HXK-J6',sec:-0.6,alliance_id:99003581,alliance:'Beyond the Breach',ours:false,adm:4.1,status:'vulnerable',when:iso(3),campaign:false,defender:'',defender_score:null,attackers_score:null,moved:false,d_def:0,d_att:0,trend:''},
  ]}
...
console.log('  OK  WIJ-badge:', (await p.locator('text=WIJ').count())>0)
await p.click('button:has-text("Van ons")'); await p.waitForTimeout(300)
console.log('  OK  Van ons-filter (alleen ours):', (await p.locator('tbody tr').count())===1)
await p.click('button:has-text("Alles")'); await p.waitForTimeout(200)
